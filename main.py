...
def _configure_observability() -> None:
    try:
        ...
        if APPLICATION_INSIGHTS_CONNECTION_STRING:
            configure_kwargs["connection_string"] = (
                APPLICATION_INSIGHTS_CONNECTION_STRING
            )
        else:
            logger.warning(
                "No Application Insights connection string provided; Azure Monitor exporter disabled."
            )
        ...
        logger.info(
            "Microsoft Agent Framework observability configured for service %s",
            SERVICE_NAME,
        )
...
def get_exchange_rate(
    currency_from: Annotated[
        str, Field(description="Base currency (3-letter ISO code).", max_length=3)
    ] = "USD",
    currency_to: Annotated[
        str, Field(description="Target currency (3-letter ISO code).", max_length=3)
    ] = "EUR",
    currency_date: Annotated[
        str, Field(description="Date to query (YYYY-MM-DD or 'latest').", max_length=10)
    ] = "latest",
) -> str:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            response = requests.get(
                endpoint, params=params, timeout=DEFAULT_TIMEOUT_SECONDS
            )
    ...
    async def startup(self) -> None:
        ...
        if not (uses_existing_agent or can_create_ephemeral):
            ...
            class _FallbackAgent:
                async def run(
                    self, prompt: str, **_: Any
                ) -> str:  # pragma: no cover - simple demo stub
                    return (
        ...
        if uses_existing_agent:
            ...
            logger.info(
                "Creating ephemeral Azure AI agent using deployment %s",
...
app = FastAPI(
    title="Azure Container Apps - Microsoft Agent Framework Sample", version="0.1.0"
)
